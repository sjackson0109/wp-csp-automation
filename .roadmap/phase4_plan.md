...
**Addresses:** `.roadmap/phase3_early_plan.md` §25 (Commercial Product Boundary), GitHub #172 (closed 2026-09-02, redirected here), #173 (closed, schema shipped), #174 (grace-period entitlement logic, still open), and `docs/sam-portal-requirements-spec.md` (rewritten 2026-09-04 as a full componentised-platform spec -- substantially larger in scope than the checkout-proxy design this line originally referenced; its own §24 states current code and this specification outrank obsolete roadmap numbering, so treat `phase4_plan.md`'s older framing below as superseded wherever the two disagree).
...
**4E.2 -- SAM Portal build** (`docs/sam-portal-requirements-spec.md`): still not started within this repository. The spec's scope grew substantially in its 2026-09-04 rewrite -- no longer just a Cloudflare Worker handling checkout, now a full three-repository platform (`vcns/security-automation-manager` this repo, `vcns/sam-licensing-service`, `vcns/sam-portal`) with its own delivery plan (§20: Foundation -> Portal minimum viable service -> Integrated SAM -> Assurance and fleet expansion). The spec's own §24 "Decisions Fixed by This Specification" has already resolved the five open decisions this line previously cited (domain/repo naming among them: `sam.vcns.tech`, `licensing.vcns.tech`, `vcns/sam-licensing-service`, `vcns/sam-portal`) -- that blocker is gone. GitHub #174's grace-period requirement is specified as part of this track (spec §15) -- building the portal/licensing integration is what actually closes #174, not a separate effort. **Observed but not verified in depth this session:** a local `vcns/sam-licensing-service` checkout already has substantial real history (Stripe subscription adoption, a KV-backed product catalogue, admin auth, and an early admin-portal shell as of PR #19) -- this appears to be active Foundation-phase progress, but it's a separate repository this session hasn't worked in, so treat that as a pointer to check, not a confirmed status.
