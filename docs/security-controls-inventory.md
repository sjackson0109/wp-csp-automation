...
- **Default state:** always active whenever CSP is active on a surface (report-only or enforce); the default transport (`wp_sam_reporting_transport`, one of `report-uri` / `report-to` / `both`) is `both` -- report-uri delivery for browsers without Reporting API support, report-to for browsers that do (which then ignore report-uri). A report-uri-only default was the original choice (fastest feedback while learning a policy) until a production incident traced an unthrottled report-uri storm -- one immediate, unbatched request per violation -- to a customer's PHP-FPM worker pool being exhausted; see schema v39's migration note and the Risk of breakage entry below.
...
- **Risk of breakage:** low for the reporting mechanism corrupting data or exposing information -- reports are accepted only for `application/csp-report`, `application/reports+json`, or `application/json` Content-Types (anything else gets HTTP 400); a report whose `document-uri` hostname doesn't match this site's own origin is silently discarded (RFC 6454 origin comparison) without revealing the rejection, specifically so the check itself isn't advertised to a probing attacker. Storage is rate-limited to 500 reports/hour per (surface, directive) and deduplicated by a SHA-256 fingerprint of surface + blocked-uri + violated-directive, so a noisy or hostile reporter cannot grow the table unbounded -- **but this storage cap alone does not bound how many requests get accepted and processed**, which is a materially different risk: a production incident traced an unthrottled report-uri violation storm (one immediate, unbatched request per violation, from a report-only policy that hadn't yet learned a page's real needs) to exactly this, exhausting a customer's PHP-FPM worker pool. `Violation_Reporter::check_not_flooding()`, wired in as the REST route's own `permission_callback`, now rejects a single IP sending more than 40 reports in 10 seconds with HTTP 429 before any of the above runs, and the default transport (`both`, see Default state above) reduces request volume at the source for browsers that support the Reporting API.
...
- **Rollback behaviour:** both `sam_pillar_profiles` (mode/enabled) and `sam_dependency_inventory` (the actual per-origin classifications) are in `Rollback_Guard::SNAPSHOT_TABLE_SUFFIXES` and so are covered by the generic pre-migration snapshot -- restorable together via the Recovery tab, same-schema restores only, and only back to whatever the most recent snapshot happened to capture (not to any arbitrary prior state, and not as a per-classification undo).
